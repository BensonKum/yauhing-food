...
async function renderReport(){
  ...
  // setDateRange: 設定日期範圍（今日 / 自訂）
  const setDateRange = (mode) => {
    const today = new Date();
    const fmt = d => d.toISOString().split('T')[0];
    if(mode==='today'){
      document.getElementById('logFromDate').value = fmt(today);
      document.getElementById('logToDate').value = fmt(today);
    }
    renderLog();
  };
  // onDateRangeChange: 自訂日期輸入變化時自動刷新
  const onDateRangeChange = () => {
    ...
    if(from && to){
      ...
      const diff = (tDate-fDate)/(1000*60*60*24);
      if(diff > 93){
        alert('日期範圍不可超過3個月');
        return;
      }
...
async function renderLog(){
  ...
  // 今日 mode：只顯示今日
  const logDateFrom = document.getElementById('logFromDate') ? document.getElementById('logFromDate').value : '';
  const logDateTo = document.getElementById('logToDate') ? document.getElementById('logToDate').value : '';
  ...
  const inDateRange = (t) => {
    ...
    return true; // 今日 mode：由 renderLog 處理
  };
  ...
  document.getElementById('logSidebar').innerHTML =
    ...
    '<div class="log-card" style="background:#fff;border:1px solid #e0e0e0">' +
      '<div class="label" style="margin-bottom:.4rem;font-size:.75rem;color:#999">自訂日期範圍</div>' +
      '<div style="display:flex;flex-direction:column;gap:.3rem">' +
        '<label style="font-size:.7rem;color:#888">由</label>' +
        '<input type="date" id="logFromDate">' +
        '<label style="font-size:.7rem;color:#888">至</label>' +
        '<input type="date" id="logToDate">' +
        '<button onclick="onDateRangeChange()" style="margin-top:.3rem;padding:.4rem;background:#2196f3;color:#fff;border:none;border-radius:4px;font-size:.85rem;cursor:pointer">確定</button>' +
      '</div>' +
    '</div>';
...
window.setDateRange = function(mode) {
  // Clear custom date inputs, set quickDays mode, re-render
