...
async function renderLog(){
  ...
  // 今日 mode：只顯示今日
  const logDateFrom = document.getElementById('logDateFrom') ? document.getElementById('logDateFrom').value : '';
  const logDateTo = document.getElementById('logDateTo') ? document.getElementById('logDateTo').value : '';
  ...
  document.getElementById('logSidebar').innerHTML =
    '<div class="log-card" style="background:#fff;border-left:4px solid #bbb;height:55px"></div>' +
    ...
    '<div class="log-card" style="background:#fff;border:1px solid #e0e0e0">' +
      ...
      '<div style="display:flex;flex-direction:column;gap:.3rem">' +
        '<input type="date" id="logDateFrom" onchange="onDateRangeChange()">' +
        '<div style="text-align:center;font-size:.7rem;color:#bbb">至</div>' +
        '<input type="date" id="logDateTo" onchange="onDateRangeChange()">' +
      '</div>' +
    '</div>';
