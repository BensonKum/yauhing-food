...
function showPinLogin(){
  ...
  pinInput.value = '';
  if(!pinInput.dataset.pinBound){
    pinInput.dataset.pinBound = '1';
    ...
    pinInput.addEventListener('keydown', function(e){
      if(e.key === 'Enter') pinLogin();
    });
  }
  pinInput.focus();
}

// (moved below with resetForm support)
...
function sendResetEmail(){
  ...
  if(!email){document.getElementById('resetError').textContent='請輸入電郵';return;}
  ...
  auth.sendPasswordResetEmail(email).then(function(){
    ...
    document.getElementById('resetForm').querySelector('button[onclick^="showEmailLogin"]').style.display='none';
    var msg=document.createElement('div');
    msg.style.cssText='color:var(--green);font-size:.8rem;text-align:center;margin-top:.8rem';
    msg.textContent='重設連結已發到你的電郵，請查收。';
    document.getElementById('resetForm').appendChild(msg);
  }).catch(function(err){
    btn.disabled=false;btn.textContent='發送重設電郵';
    var msgs={'auth/user-not-found':'此電郵未註冊','auth/invalid-email':'電郵格式不正確'};
    document.getElementById('resetError').textContent=msgs[err.code]||'發送失敗：'+err.message;
  });
}
...
async function showApp(){
  ...
  // Admin emails: Jimmy and 容
  if(email === 'jyvivi618@yahoo.com.hk' || email === 'inamyleung@yahoo.com.hk' || email === 'admin@yauhing.hk'){
    isAdminUser = true;
    isSMUser = false;
    showEmployeeTab();
    showStoreSelect();
...
function setMode(mode){
  ...
  const invoiceArea=document.getElementById('invoiceUploadArea');
  if(mode==='sale'){
...
function changeQty(cartKey,delta,p,selPack){
  ...
  if(!cart[cartKey]){
    ...
    // Manual price for YHM01 (現金收入) and YHM02 (外購)
    if(String(p.name||'').includes('現金')||String(p.name||'').includes('外購')){
      const mp=prompt('請輸入實際金額（'+p.name+'）\n（預設：'+cart[cartKey].price+'）','');
      if(mp!==null&&mp!==''){
        const mv=parseFloat(mp);
        if(!isNaN(mv)&&mv>0){cart[cartKey].manualPrice=mv;}
      }
    }
  }
...
function renderCart(){
  ...
  keys.forEach(cartKey=>{
    ...
    const lineTotal=item.qty*(item.manualPrice||item.price);
    total+=lineTotal;count+=item.qty;
    const displayName=item.pack?`${item.name}（${item.pack}）`:item.name;
    html+=`<div class="cart-item">
      <div class="ci-info"><div class="ci-name">${displayName}</div><div class="ci-price">HK$${(item.manualPrice||item.price)}/件</div></div>
...
function confirmTransaction(){
  ...
  const items=Object.entries(cart).map(([name,d])=>
    `${name} × ${d.qty} = HK$${d.qty*(d.manualPrice||d.price)}`
  ).join('\n');
  const total=Object.values(cart).reduce((s,i)=>s+i.qty*(i.manualPrice||i.price),0);
  ...
  try {
    ...
    await db.collection('inventory_transactions').add({
      ...
      items:Object.entries(cart).map(([cartKey,d])=>({
        name:d.name, pack:d.pack||null, qty:d.qty, price:(d.manualPrice||d.price), lineTotal:d.qty*(d.manualPrice||d.price)
      })),
...
function showEmployeeTab(){ 
  var btn = document.getElementById('tabEmployee');
  if(btn) btn.style.display = '';
  document.getElementById('tabContentEmployee').style.display = 'block'; 
}
function hideEmployeeTab(){ 
  var btn = document.getElementById('tabEmployee');
  if(btn) btn.style.display = 'none';
  document.getElementById('tabContentEmployee').style.display = 'none'; 
}
...
document.getElementById('logList').innerHTML='<div style="text-align:center;padding:3rem;color:var(--muted)">暫無交易記錄<br><small>在POS頁面完成交易後，記錄會顯示在這裡</small></div>';
  ...
  // Group transactions: same store + same source + within 1 minute = same invoice/batch
  const groups = [];
  window._logGroups = groups;
  txns.forEach(t => {
    ...
    let group = groups.find(g => 
      g.store === store && 
      g.source === source && 
      Math.abs(g.dtMs - dtMs) < 60000 // within 1 minute
    );
    if(!group){
      const gid = 'g_' + Math.random().toString(36).substr(2,9);
      group = { dt, dtMs, store, source, txns: [], total: 0, gid: gid };
      groups.push(group);
    }
    group.txns.push(t);
    group.total += t.total || 0;
  });
  
  // Render grouped entries
  document.getElementById('logList').innerHTML=groups.map(g=>{
    const dtStr=g.dt.toLocaleString('zh-HK',{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'});

  // Event delegation for log group toggle
  document.getElementById('logList').addEventListener('click', function(e){
    const entry = e.target.closest('.log-group-toggle');
    if(entry){
  ...
  document.getElementById('invoicePreview').addEventListener('click', function(e){
    ...
    // Combine all items from grouped transactions
    let allItems = [];
    g.txns.forEach(t => {
      if(t.items && t.items.length > 0){
        allItems = allItems.concat(t.items.map(i=>`${i.name}${i.pack?'('+i.pack+')':''} × ${i.qty}`));
      } else if(t.productName){
        allItems.push(`${t.productName} × ${t.qty || 0}`);
      }
    });
    const itemsStr = allItems.join(', ');
    const itemCount = allItems.length;
    const gid = g.gid;
    const adminBtns = (isAdminUser || isSMUser) ? '<button class="log-edit-btn" onclick="editTransaction(\'' + gid + '\')">✏️</button><button class="log-del-btn" onclick="deleteTransaction(\'' + gid + '\')">🗑️</button>' : '';
    return '<div class="log-entry log-group-toggle" data-gid="'+gid+'">' +
      '<div style="flex:1">' +
        '<div class="log-meta">' + dtStr + ' · <span class="log-type ' + type + '">' + (type==='sale'?'📤 銷售':'📥 進貨') + '</span> · ' + (store==='central'?'中環':'始創') + ' · ' + user + '</div>' +
        '<div class="log-items" style="cursor:pointer">📦 ' + itemCount + '款產品 (click to expand)</div>' +
      '</div>' +
      '<div style="display:flex;align-items:center;gap:.5rem;">' +
        '<div class="log-total">HK$' + g.total + '</div>' +
        adminBtns +
      '</div>' +
      '<div id="'+gid+'" style="display:none;width:100%;background:#f5f5f5;border-radius:8px;padding:.5rem;margin-top:.5rem;font-size:.75rem;color:#666">' + itemsStr + '</div>' +
    '</div>';
  }).join('');
}
...
// Delete transaction - batch delete with custom confirmation
window.deleteTransaction = function(gid) {
  // Find the group by gid
  const groups = window._logGroups || [];
  ...
  group.txns.forEach(t => {
    const type = t.type === 'sale' ? '銷售' : '進貨';
    const user = t.user || '—';
    msg += `- ${type} HK$${t.total} (${user})\n`;
  });
  msg += '\n合計：HK$' + group.total + '\n\n此操作不可還原！';
  ...
  const batch = db.batch();
  group.txns.forEach(t => {
    batch.delete(db.collection('inventory_transactions').doc(t.id));
  });
  batch.commit().then(function() {
    // Update inventory (reverse the transactions)
    const updatePromises = [];
    group.txns.forEach(t => {
      const delta = t.type === 'sale' ? 1 : -1;  // sale: add back; purchase: subtract
      t.items.forEach(item => {
        const pName = item.name;
        const qty = item.qty;
        const ref = db.collection('inventory').doc(pName);
        // Use serverTimestamp to increment/decrement
        const update = {};
        update[t.store] = firebase.firestore.FieldValue.increment(qty * delta);
        updatePromises.push(ref.set(update, { merge: true }));
      });
    });
    Promise.all(updatePromises).then(function() {
      alert('已刪除 ' + group.txns.length + ' 筆交易，庫存已還原');
      renderLog();
    }).catch(function(e) {
      console.error('Inventory update failed:', e);
      alert('交易已刪除，但庫存還原失敗：' + e.message);
      renderLog();
    });
  }).catch(function(e) {
    console.error(e);
...
async function handleInvoiceUpload(input){
  ...
  try{
    ...
    // Parse invoice data (rows 22-29 typically contain products)
    const invoiceItems=[];
    let invoiceNumber='';
    let customerName='';
    
    // Find invoice number and customer
    for(let i=0;i<jsonData.length;i++){
      const row=jsonData[i];
      if(row[0] && String(row[0]).includes('發票號碼')){
        invoiceNumber=String(row[1]||row[2]||'').trim();
      }
      if(row[0] && String(row[0]).includes('客戶名稱')){
        customerName=String(row[1]||row[2]||'').trim();
      }
    }
    
    // Detect store from customer name
    let targetStore=currentStore;
    if(customerName.includes('中環')) targetStore='central';
    else if(customerName.includes('始創')) targetStore='pioneer';
    
    // Parse product rows (flexible column detection)
    for(let i=0;i<jsonData.length;i++){
      const row=jsonData[i];
      if(!row || row.length<3) continue;
      
      // Try to find SKU pattern (YHxxx) in any column
      let sku='';
      let name='';
      let qty=0;
      ...
      for(let col=0;col<row.length;col++){
        ...
        // Check for SKU pattern (YH followed by digits)
        if(!sku && /^YH\d+[A-Z]?$/i.test(strValue)){
          sku=strValue.toUpperCase();
        }
        // Check for quantity (number that looks like quantity)
        else if(!qty && !isNaN(parseFloat(strValue)) && parseFloat(strValue)>0 && parseFloat(strValue)<10000){
          // Make sure it's not the price (usually larger or has decimal)
          const numVal=parseFloat(strValue);
          if(numVal<1000 || Number.isInteger(numVal)){
            qty=Math.floor(numVal);
          }
        }
        // Check for product name (contains Chinese characters and is longer)
        else if(!name && strValue.length>3 && /[\u4e00-\u9fa5]/.test(strValue) && !strValue.includes('發票') && !strValue.includes('客戶')){
          name=strValue;
        }
      }
      ...
      if(sku && name && qty>0){
        ...
        const product=products.find(p=>p.sku===sku || p.name===name || name.includes(p.name) || p.name.includes(name.substring(0,5)));
        if(product){
          invoiceItems.push({
            sku:sku,
            name:product.name,
    ...
    if(invoiceItems.length===0){
      preview.textContent='❌ 未能識別發票產品資料';
      return;
    }
    
    // Show preview
    let previewHtml=`[OK] 識別到 ${invoiceItems.length} 款產品<br>`;
    previewHtml+=`<small>發票：${invoiceNumber || 'N/A'} | 客戶：${customerName || 'N/A'}</small><br>`;
    previewHtml+=`<div style="max-height:100px;overflow-y:auto;margin-top:.4rem;text-align:left;background:#f5f5f5;padding:.4rem;border-radius:6px;">`;
...
async function confirmInvoiceRestock(items,targetStore){
  if(!targetStore){
    toast('請先選擇分店');
    return;
  }
  
  if(!confirm(`確認將 ${items.length} 款產品入庫到 ${targetStore==='central'?'中環店':'始創店'}？`)) return;
  ...
  try{
    ...
    for(const item of items){
      // Update inventory
      const invRef=db.collection('inventory').doc(item.name);
      batch.set(invRef,{
        [targetStore]:firebase.firestore.FieldValue.increment(item.qty),
        lastUpdated:timestamp
      },{merge:true});
      
      // Add transaction record
      const txnRef=db.collection('inventory_transactions').doc();
      batch.set(txnRef,{
        type:'purchase',
    ...
    // Refresh inventory + transaction log
    await loadInventory();
    await renderLog();
    
    document.getElementById('invoicePreview').innerHTML='✅ 入庫成功！已更新 '+items.length+' 款產品';
    toast('發票入庫完成');
    
  }catch(err){
    console.error('Restock error:',err);
    toast('入庫失敗：'+err.message);
  }
}

window.handleInvoiceUpload=handleInvoiceUpload;
window.confirmInvoiceRestock=confirmInvoiceRestock;
